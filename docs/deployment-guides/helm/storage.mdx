...
<Note>
With ClickHouse, `storage.logsStore.retentionDays` (rendered as `retention_days`) is enforced by a native table **TTL**. The PostgreSQL-only `matviewRefreshInterval` setting has no effect on ClickHouse.
</Note>
