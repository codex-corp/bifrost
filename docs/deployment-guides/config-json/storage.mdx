...
<Note>
With ClickHouse, `retention_days` is enforced by a native table **TTL** rather than a background delete job. `matview_refresh_interval` does not apply — it is a PostgreSQL-only setting for materialized views.
</Note>
...
<Note>
In `config.json` each store carries its own `config` block, so the two stores can point at entirely separate hosts. (The Helm chart shares a single PostgreSQL connection across both stores — to split them across instances, configure `config.json` directly.)
</Note>
